...
public class Modelo {
	...
	public Modelo(){
		...
		if(monedaDAO.isEmpty()) {
			monedaDAO.guardarMoneda (new Cripto(0,".\\Logos\\Bitcoin_logo.png" , "BITCOIN","BTC",0, 0, 0));
			monedaDAO.guardarMoneda(new Cripto(0,".\\Logos\\Ethereum_logo.png" , "ETHEREUM","ETH",0, 0, 0));
			monedaDAO.guardarMoneda(new Cripto(0,".\\Logos\\Dogecoin_Logo.png" , "DOGECOIN","DOGE",0, 0, 0));
			monedaDAO.guardarMoneda(new Cripto(0,".\\Logos\\Tether_logo.png" , "TETHER","USDT",0, 0, 0));
			monedaDAO.guardarMoneda(new Cripto(0,".\\Logos\\Usdc_logo.png" , "USD-COIN","USDC",0, 0, 0));
		}
	}	
	...
	public List<Activo> listarActivosTipo(int tipo, int id_usuario){
		if(tipo == FIAT)
			return activoDAO.listarActivosFiat(id_usuario);
		if(tipo == CRIPTO)
			return activoDAO.listarActivosCripto(id_usuario);
		return null;
	}
